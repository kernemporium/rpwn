...
fn check_sec_64(elf : &ElfFile){
    ...
    for section in elf.section_iter(){
        ...
        match section.get_data(&elf){
            //la section est bien un dynamic 64
            Ok(sections::SectionData::Dynamic64(dynamic_tab))=>{
                //on prend les tags de cette section
                for dynamic in dynamic_tab{
                    match dynamic.get_tag(){
                        Ok(dynamic::Tag::Flags1) => {
